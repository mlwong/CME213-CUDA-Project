...
// Kernel to compute the softmax function
__global__
void device_softmax (const double* const d_mat_1,
					 double* d_mat_2,
					 double* d_mat_vec_exp,
					 const int m)
{
	// Global thread index in the grid
	const int tid_x = threadIdx.x + blockDim.x*blockIdx.x;
	...
	// Initialize the column vector d_mat_sum_exp
	if (tid_y == 0)
	{
		d_mat_vec_exp[tid_x] = d_mat_2[idx];
	}
	...
	d_mat_2[idx] = d_mat_2[idx] / d_mat_vec_exp[tid_x];
}
                   ...
                   const int n)
{
	double *d_mat_1;
	double *d_mat_2;
	double *d_vec_sum_exp;
	
	// Allocate the device memory
	checkCudaErrors(cudaMalloc(&d_mat_1, m*n*sizeof(double)));
	checkCudaErrors(cudaMalloc(&d_mat_2, m*n*sizeof(double)));
	
	checkCudaErrors(cudaMalloc(&d_vec_sum_exp, m*sizeof(double)));
	...
	// Compute the block dimension, each block compute softmax of one row vector
	n_threads.x = 1;
	n_threads.y = n;
	...
	device_softmax <<<n_blocks, n_threads>>> (d_mat_1, d_mat_2, d_vec_sum_exp, m);
