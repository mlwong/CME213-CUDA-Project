...
std::vector<uint> computeBlockHistograms(const std::vector<uint> &keys, uint numBlocks,
                                         uint numBuckets, uint numBits, 
                                         uint startBit, uint blockSize);

std::vector<uint> reduceLocalHistoToGlobal(const std::vector<uint>& blockHistograms,
...
void populateOutputFromBlockExScan(const std::vector<uint>& blockExScan, uint numBlocks,
...
void Test1() { 
  ...
  std::vector<uint> blockHistograms = computeBlockHistograms(input, numBlocks, numBuckets, kSizeMaskTest, kStartBitTest, blockSize);
  bool success = true;
...
void Test5() { 
  ...
  populateOutputFromBlockExScan(blockExScan, numBlocks, numBuckets, kStartBitTest, kSizeMaskTest, blockSize, input, sorted);
  bool success = true;
