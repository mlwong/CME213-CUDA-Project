...
Grid::Grid(const Grid &other) : gx_(other.gx()), gy_(other.gy()), hGrid_(other.hGrid_) {
  cudaError_t err = cudaMalloc(&dGrid_, gx_ * gy_ * sizeof(float));
  if (err != cudaSuccess) {
  ...
  cudaMemcpy(dGrid_, other.dGrid_, gx_ * gy_ * sizeof(float), cudaMemcpyDeviceToDevice);
}

//copy from host -> device
void Grid::toGPU() {
  cudaError_t err = cudaMemcpy(dGrid_, &hGrid_[0], gx_ * gy_ * sizeof(float), cudaMemcpyHostToDevice);
  if (err != cudaSuccess) {
...
void Grid::fromGPU() {
  cudaError_t err = cudaMemcpy(&hGrid_[0], dGrid_, gx_ * gy_ * sizeof(float), cudaMemcpyDeviceToHost);
  if (err != cudaSuccess) {
...
std::ostream& operator<<(std::ostream& os, const Grid &grid) {
  ...
  for (int y = 0; y < grid.gy(); ++y) {
    for (int x = 0; x < grid.gx(); x++) {
      os << setw(5) << x << " " << setw(5) << y << " " << setw(15) << grid.hGrid_[x + grid.gx() * y] << endl;
    }
    os << endl;
  }
  os << endl;
