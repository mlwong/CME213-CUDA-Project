...
void setBCs(Grid &grid, const simParams &params) {
  //set BCs
...
template<int order>
inline float stencil(float *curr_grid, int gx, int x, int y, float xcfl, float ycfl) {
  if (order == 2) {
    return curr_grid[x + gx * y] + 
    xcfl * (curr_grid[x+1 + gx * y] + curr_grid[x-1 + gx * y] - 2 * curr_grid[x + gx *  y]) + 
    ycfl * (curr_grid[x + gx *(y+1)] + curr_grid[x + gx *(y-1)] - 2 * curr_grid[x + gx * y]);
  }
  else if (order == 4) {
    return curr_grid[x + gx * y] + 
    xcfl * (   -curr_grid[x+2 + gx * y] + 16 * curr_grid[x+1 + gx * y] -
     30 * curr_grid[x + gx * y] + 16 * curr_grid[x-1 + gx * y] - curr_grid[x-2 + gx *  y]) + 
    ycfl * (   -curr_grid[x +  gx * (y+2)] + 16 * curr_grid[x + gx * (y+1)] -
     30 * curr_grid[x + gx * y] + 16 * curr_grid[x + gx * (y-1)] - curr_grid[x + gx * (y-2)]);
  }
  else if (order == 8) {
    return curr_grid[x + gx * y] + 
    xcfl * (   -9*curr_grid[x+4 + gx * y] + 128 * curr_grid[x+3 + gx *  y] - 1008 * curr_grid[x+2 + gx * y] + 8064 * curr_grid[x+1 + gx *  y] -
     14350 * curr_grid[x + gx *  y] +
     8064 * curr_grid[x-1 + gx * y] - 1008 * curr_grid[x-2 + gx * y] + 128 * curr_grid[x-3 + gx *  y] -9 * curr_grid[x-4 + gx * y]) +
    ycfl * (   -9*curr_grid[x + gx * (y+4)] + 128 * curr_grid[x + gx * (y+3)] - 1008 * curr_grid[x + gx * (y+2)] + 8064 * curr_grid[x + gx * (y+1)] -
     14350 * curr_grid[x + gx * y] + 
     8064 * curr_grid[x + gx * (y-1)] - 1008 * curr_grid[x + gx * (y-2)] + 128 * curr_grid[x + gx * (y-3)] - 9 * curr_grid[x + gx * (y-4)]);
  }
  else {
    ...
    if (params.order() == 2) {
      for (int y = borderSize; y < ny + borderSize; ++y) {
        for (int x = borderSize; x < nx + borderSize; ++x) {
          next_grid.hGrid_[x + gx *  y] = stencil<2>(curr_grid.hGrid_.data(), gx, x, y, xcfl, ycfl);
        }
      }
    }
      ...
      for (int y = borderSize; y < ny + borderSize; ++y) {
        for (int x = borderSize; x < nx + borderSize; ++x) {
          next_grid.hGrid_[x + gx *  y] = stencil<4>(curr_grid.hGrid_.data(), gx, x, y, xcfl, ycfl);
        }
      }
    }
      ...
      for (int y = borderSize; y < ny + borderSize; ++y) {
        for (int x = borderSize; x < nx + borderSize; ++x) {
          next_grid.hGrid_[x + gx *  y] = stencil<8>(curr_grid.hGrid_.data(), gx, x, y, xcfl, ycfl);
        }
      }
    }
...
int checkErrors(const Grid &ref_grid, const Grid &gpu_grid, const simParams &params, std::string filename, std::vector<double>& errors)
{
  ...
  for (int x = 0; x < params.gx(); ++x) {
    for (int y = 0; y < params.gy(); ++y) {
      if (!AlmostEqualUlps(ref_grid.hGrid_[y * params.gx() + x], gpu_grid.hGrid_[x + params.gx() * y], 512)) {
        ofs << "Mismatch at pos (" << x << ", " << y << ") cpu: " 
        << ref_grid.hGrid_[x + params.gx() * y] << " gpu: "    
        << gpu_grid.hGrid_[y * params.gx() + x] << endl;
        ++error;
      }   
      l2ref += ref_grid.hGrid_[y * params.gx() + x] * ref_grid.hGrid_[y * params.gx() + x]; 
      l2err += (ref_grid.hGrid_[y * params.gx() + x] - gpu_grid.hGrid_[x + params.gx() * y]) 
      * (ref_grid.hGrid_[y * params.gx() + x] - gpu_grid.hGrid_[x + params.gx() * y]);

      if (ref_grid.hGrid_[y * params.gx() + x] != 0) {
       curr = abs((ref_grid.hGrid_[y * params.gx() + x] - gpu_grid.hGrid_[x + params.gx() * y]) 
        / (ref_grid.hGrid_[y * params.gx() + x])); }

      if (curr > linf) linf = curr;
    }   
  }
...
void PrintErrors(const std::vector<double>&errorsg, const std::vector<double>&errorsb, const std::vector<double>&errorss) {
  cout << endl;
  cout << setw(15) << " " << setw(15) << "L2Ref" << setw(15) << "LInf" << setw(15) << "L2Err" << endl;
  if (errorsg.size() > 0){ 
...
int main(int argc, char *argv[])
{
  ...
  if (doGlobal) {
    ...
    setBCs(gpuGrid, params); // Initialize the grid
